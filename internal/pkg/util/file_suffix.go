...
func bytesToHexString(src []byte) string {
	...
	for _, v := range src {
		sub := v & 0xFF // 低八位
		hv := hex.EncodeToString(append(temp, sub))  // 将低八位转换为十六进制字符串
		if len(hv) < 2 {
			res.WriteString(strconv.FormatInt(int64(0), 10)) // 如果长度小于2，则写入0保持格式一致
		}
		res.WriteString(hv) // 将低八位转换为十六进制字符串写入结果
	}
	return res.String()
}
...
// 用文件前面几个字节来判断
// fSrc: 文件字节流（就用前面几个字节）
// 转为16进制后，低八位处理，然后跟map中key去对应
func GetFileType(fSrc []byte) string {
	var fileType string
	fileCode := bytesToHexString(fSrc) //字节切片转换为十六进制字符串

	fileTypeMap.Range(func(key, value interface{}) bool {
		k := key.(string)
		v := value.(string)
		// 判断文件头是否以 k 开头，或者 k 是否以 fileCode 开头
