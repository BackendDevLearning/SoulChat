...
type GroupMemberTB struct {
	...
	UserID  uint32 `gorm:"column:user_id;type:int(10) unsigned;not null;uniqueIndex:idx_user_group;comment:用户ID" json:"userId"`
	GroupID uint32 `gorm:"column:group_id;type:int(10) unsigned;not null;uniqueIndex:idx_user_group;comment:群组ID" json:"groupId"`
...
type GroupMemberRepo interface {
	...
	IsUserInGroup(UserId uint32, GroupId uint32) (bool, error)
	IsUserAdmin(UserId uint32, GroupId uint32) (bool, error)
}
